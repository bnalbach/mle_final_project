#!/usr/bin/env bash
# b4 (final long run): teach HUNTING, which mode3_model never learned (it only
# ever saw "no opponents" then "1 rule_based" -- every opponent could bomb back,
# so it learned to avoid opponents, never to kill them).
#
# Base = mode3_model (cleanest, best-trained foundation, no reckless habits):
#   cp agent_code/user_agent/mode3_model/dqn_model.pt agent_code/agent_b4/dqn_model.pt
#   bash train_b4.sh
#
# Curriculum:
#   1. HUNT SAFELY  vs non-bombing agents (peaceful/coin_collector) -- learn to
#      corner+bomb+escape+kill WITHOUT the suicide penalty drowning the lesson.
#   2. REAL DANGER  vs 2 rule_based -- opponents now fight back.
#   3. FULL DENSITY vs 3 rule_based -- tournament conditions.
#   4. ROBUSTNESS   vs a mixed field incl. your old model -- don't overfit rule_based.
#
# Each stage saves every round (Ctrl-C safe), then snapshots + quick-evals so you
# keep the BEST checkpoint, not blindly the last one.
set -e
AGENT=user_agent
OLD=training_partner          # old model (b4) as a sparring opponent
mkdir -p snapshots results

stage () {  # $1=name $2=eps $3=rounds  rest=opponents
  local name="$1"; local eps="$2"; local rounds="$3"; shift 3
  echo ">>> $name: eps=$eps rounds=$rounds vs $*"
  INIT_EPSILON="$eps" python main.py play --no-gui --scenario classic \
      --train 1 --n-rounds "$rounds" --agents "$AGENT" "$@"
  cp agent_code/$AGENT/dqn_model.pt snapshots/b4_${name}.pt
  echo ">>> eval after $name"
  python evaluate.py --agent $AGENT --suite combat --rounds 50 --seed 1 \
      --timeout 0.5 --out results/b4_${name}.json
}

# 1. learn to hunt against killable, non-bombing targets (the missing stage)
stage hunt_safe   0.30  8000 peaceful_agent coin_collector_agent coin_collector_agent
# 2. real danger: opponents that bomb back
stage danger_2rule 0.20 10000 rule_based_agent rule_based_agent
# 3. tournament density
stage full_3rule   0.12 16000 rule_based_agent rule_based_agent rule_based_agent
# 4. robustness: mixed field closer to the real tournament
stage mixed_3opp   0.10 12000 rule_based_agent coin_collector_agent "$OLD"

echo ">>> DONE. Compare each snapshot against b1 and keep the best:"
for st in hunt_safe danger_2rule full_3rule mixed_3opp; do
  echo "    python compare.py results/b1.json results/b4_${st}.json"
done
echo "    # to use a snapshot:  cp snapshots/b4_<best>.pt agent_code/$AGENT/dqn_model.pt"