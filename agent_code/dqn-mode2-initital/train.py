...
# When switching curriculum mode (checkpoint's saved training_mode differs
# from the current TRAINING_MODE), epsilon is boosted to at least this value
# instead of resuming at whatever low epsilon the previous mode had
# converged to. New reward signals become active at each mode transition
# (crate/bomb rewards in mode 2, opponent rewards in mode 3) that the agent
# has never explored under, so it needs renewed exploration to discover
# them -- but a full reset to EPSILON_START would needlessly discard the
# already-learned navigation/coin-seeking exploitation benefit.
...
# Movement-diversity penalty. was_backtrack only catches the strict
...
def setup_training(self):
    ...
    # so stopping and restarting training continues the exploration decay
    # schedule instead of resetting to EPSILON_START every run.
    ...
    if os.path.isfile(MODEL_PATH):
        ...
        if checkpoint_mode != TRAINING_MODE:
            # Curriculum stage changed since this checkpoint was saved --
            # new reward signals are now active (crate/bomb rewards in mode
            # 2, opponent rewards in mode 3) that the agent has never been
            # trained against. Boost exploration so it actually discovers
            # the newly-relevant actions/outcomes, without fully discarding
            # the exploitation value of the already-learned policy via a
            # full reset to EPSILON_START.
...
def reward_from_transition(
    ...
    # --------------------------------------------------------------------
    # Danger status at the *start* of this transition -- computed once,
    # up front, so it can gate both the coin- and crate-distance shaping
    # further down (neither should compete with escape behaviour while
    # the agent is standing somewhere a bomb could hit it).
    # old_timer == 99 means "no bomb currently threatens this position".
    # --------------------------------------------------------------------
    ...
    # --------------------------------------------------------------------
    # Crate destruction -- credited on WHICHEVER step CRATE_DESTROYED
    # actually appears in events. This must NOT be nested inside the
    # "self_action == BOMB" branch below: BOMB_DROPPED and CRATE_DESTROYED
    # happen on different steps (the crate explodes several steps after
    # the bomb is placed, once BOMB_TIMER elapses), so by the time the
    # explosion/CRATE_DESTROYED event arrives, self_action for that step
    # is whatever the agent did then (often a move, or WAIT) -- not BOMB.
    # Gating this on self_action == "BOMB" meant it could only ever fire
    # on the rare coincidence of dropping a new bomb on the exact same
    # step an old one detonated, which is why crates_destroyed_this_round
    # stayed at 0 even with well-targeted bombs landing.
    # --------------------------------------------------------------------
    ...
    # Only count/reward an actual bomb placement -- self_action == "BOMB"
    # merely records what the agent *attempted*; the engine only emits
    # BOMB_DROPPED when a bomb was genuinely placed (e.g. bombs_left was
    # > 0 at the time). Without this check, repeated BOMB attempts that
    # the engine silently rejects (agent already has a live bomb) would
    # still be counted as distinct bombs and re-evaluated against the same
    # unchanged game state, inflating bombs_dropped_this_round far beyond
    # what's physically possible given a 1-bomb-at-a-time capacity.
    ...
    # --------------------------------------------------------------------
    # Coin-distance shaping -- suppressed while the agent is in a blast
    # danger zone (old_timer != 99), so it doesn't compete with the
    # escape-related rewards further down during a flee.
    # --------------------------------------------------------------------
    ...
    # Also suppressed while the agent is in a blast danger zone, for the
    # same reason as the coin-distance shaping above.
    # --------------------------------------------------------------------
    ...
    # Penalise low movement diversity directly, independent of the exact
    # oscillation pattern (catches A-B-C-B loops that was_backtrack misses,
    # without double-penalising the strict A-B-A case captured above --
    # both can fire together for a true 2-tile pendulum, which is
...
def end_of_round(self, last_game_state, last_action, events):
    ...
    self.logger.info(
        ...
        "bombs=%d; crates=%d; useless_bombs=%d",
        self.training_rounds,
        TRAINING_MODE,
        self.epsilon,
    ...
    # Keep a one-round-old backup before overwriting, in case the process is
    # killed (e.g. Ctrl+C) mid-write and corrupts the primary checkpoint.
    if os.path.isfile(MODEL_PATH):
        try:
