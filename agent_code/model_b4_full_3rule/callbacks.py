...
MODEL_PATHS = [
    os.path.join(os.path.dirname(__file__), "dqn_model.pt"),
    os.path.join(os.path.dirname(__file__), "b4_full_3rule.pt"),
]
...
def state_to_features(game_state, recent_positions=None):
    ...
    # Appended mode-3 inputs preserve the ordering of all old features.
...
def load_compatible_model_state(model, checkpoint, logger=None):
    """Load an older checkpoint after appended state features."""
...
def setup(self):
    ...
    model_path = next((path for path in MODEL_PATHS if os.path.isfile(path)), None)
    if model_path is None:
        raise FileNotFoundError(f"No model checkpoint found; tried {MODEL_PATHS}")

    checkpoint = torch.load(model_path, map_location=DEVICE)
    load_compatible_model_state(self.policy_net, checkpoint, self.logger)
    self.logger.info("Loaded model checkpoint: %s", model_path)
