...
def state_to_features(game_state):
    ...
    # Directional safe-reachability features (for general movement)
...
def act(self, game_state):
    ...
    if self.train and np.random.random() < self.epsilon:
        ...
        # escape route. This does not change what the network can learn
        # to value -- it only stops random rollouts from being dominated
        # by instant-death bomb drops before any useful signal is collected.
    ...
    return next(action for action in ACTIONS if action in best_actions)
