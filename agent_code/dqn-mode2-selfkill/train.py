...
PENALTY_KILLED_SELF = -60.0  # raised from -15.0: at BOMB_TIMER=4 steps, the
# discounted penalty must outweigh the crate rewards typically banked by a
# bomb that destroys several crates (a 5-crate bomb was still net-positive
# to die for at -15).
...
if TRAINING_MODE >= 2:
    ...
    PENALTY_BOMB_NO_SAFE_ESCAPE = -12.0  # raised from -3.0: this fires
    # immediately at bomb-placement time (no discounting), so it's the
    # cheapest, most direct lever to make a bomb with no safe escape
    # unattractive even when it would hit several crates.
    ...
    # Direct, immediate reward/penalty for choosing (or ignoring) a
    # direction already known to be a safe escape from an active bomb.
...
def _points_toward(game_state, action, target):
    """True if `action` is the first step of the shortest WALKABLE path from
    the agent's current position to `target`.
    """
    if target is None:
        return False
...
def _bomb_direction_guidance_reward(old_game_state, old_position, self_action):
    """Direct, immediate reward/penalty for whether the chosen move actually
    heads into a direction already known (via the per-direction safety
    features) to be a safe escape from a currently active bomb.

    Only fires when the agent is currently inside an active bomb's blast
    radius (old_timer < 99), the action taken is an actual move (not BOMB or
    WAIT), and at least one direction is a genuine safe escape (if none are,
    the situation is already unrecoverable and attributable to an earlier
    bomb-placement mistake, not to this move).
    """
...
def reward_from_transition(
    ...
    # Immediate feedback on whether the movement actually taken while
    # inside an active bomb's blast radius matched a direction the safety
    # features already flagged as a safe escape.
