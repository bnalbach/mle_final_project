...
def _direction_has_safe_reachable_after_bomb(game_state, position, direction):
    field = game_state["field"]
    dx, dy = MOVE_DELTAS[direction]
...
def _direction_distance(start, target, distance):
    if target is None:
        return 0, 0

    dx = np.clip(target[0] - start[0], -1, 1)
    dy = np.clip(target[1] - start[1], -1, 1)

    if abs(dx) >= abs(dy) and dx:
        direction = 1 if dx < 0 else 2
    elif dy:
        direction = 3 if dy < 0 else 4
    else:
        direction = 0
...
def _recently_visited_directions(position, recent_positions):
    ...
    This gives the network an explicit, momentary signal ("this neighbour
    tile is somewhere I just was") instead of relying purely on the
    training-time backtrack/revisit reward penalties to discourage
    oscillation. Those penalties only ever influence the learned weights;
    they carry no information at inference time about *which* direction
    just led backwards. This feature closes that gap, and -- importantly --
    is computed identically whether self.train is True or False, so it also
    helps during plain evaluation/play where no exploration randomness is
    present to break an A <-> B pendulum.
    """
...
def state_to_features(game_state, recent_positions=None):
    ...
    for target, distance in [(coin, coin_distance), (crate, crate_distance)]:
        direction, level = _direction_distance(position, target, distance)
    ...
    # Directional escape features *after dropping a bomb here*. These tell
    # the network explicitly which of the four directions would still be
    # safe to flee into if it bombed right now, instead of forcing it to
    # infer escape direction indirectly from the single _bomb_has_safe_escape
    # bit.
    for direction in ["UP", "DOWN", "LEFT", "RIGHT"]:
    ...
    # Short-term movement-history features. For each direction, is the
    # neighbouring tile one the agent occupied within the last few steps?
    # This directly targets A <-> B oscillation, including in eval/play mode
    # where there is no exploration randomness to break the pattern.
...
def setup(self):
    ...
    # Short-term position memory, maintained here (not just in train.py) so
    # the oscillation-awareness feature works identically during plain
    # evaluation/play, where self.train is False and no training callbacks
    # run at all.
...
def act(self, game_state):
    if not hasattr(self, "recent_positions"):
        # Defensive fallback in case act() is ever called without setup()
        # having run first (should not normally happen).
        self.recent_positions = deque(maxlen=POSITION_HISTORY_LENGTH)
    ...
    if self.train and np.random.random() < self.epsilon:
        # Bias random exploration away from near-certain suicide: only
        # include BOMB in the exploration pool if it would leave a safe
        # escape route.
        if "BOMB" in actions and not _bomb_has_safe_escape(game_state):
            actions = [a for a in actions if a != "BOMB"]

        # Bias exploration away from pointless standing-still. If a safe,
        # reachable move exists and there is no bomb currently threatening
        # this tile, drop WAIT from the random-exploration pool.
