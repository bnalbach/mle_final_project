...
REPLAY_BUFFER_CAPACITY = 150_000  # b4: hold more combat experience for a long GPU run
BATCH_SIZE = 256  # b4: bigger batch -> more learning per (CPU-bound) sim step, ~free on GPU
...
if TRAINING_MODE >= 2:
    ...
    PENALTY_LOST_ESCAPE_ROUTE = -30.0
else:
...
if TRAINING_MODE >= 3:
    ...
    PENALTY_GOT_KILLED = -50.0          # b4: pull back toward caution (b3's -30 was too reckless)
    REWARD_BOMB_HITS_OPPONENT = 8.0     # safe bomb whose blast covers an opponent (paid at placement)
    REWARD_MOVED_TOWARD_OPPONENT = 0.08 # b4: gentler so it doesn't charge to its death
    PENALTY_USELESS_BOMB = -3.0         # bomb that hits neither crate nor opponent
else:
...
def _bomb_placement_reward(old_game_state, new_game_state, new_position):
    ...
    # b4: require a genuine escape (reach safety within BOMB_TIMER-2 moves),
    # not a just-barely one -- kills 'technically escapable, dies anyway' bombs.
    is_safe = _position_can_survive(new_game_state, new_position, BOMB_TIMER - 2)
...
def reward_from_transition(
    ...
    if self_action == "BOMB" and _bomb_actually_dropped(events):
        ...
        _threatens_opp = _opp is not None and _bomb_would_hit_position(
            old_game_state["field"], old_position, _opp
        )

        if predicted_crates == 0:
            self.useless_bombs_this_round += 1
            if not _threatens_opp:
                reward += PENALTY_USELESS_BOMB   # bomb hits nothing -> pure suicide risk

        # Offensive shaping: a SAFE bomb whose blast would catch an opponent.
        if is_safe and _threatens_opp:
